...
function after_insert($id)
{
	...
	db_query("UPDATE clients_fields SET code = 'r".($currentid["val"]+1)."' WHERE id = '$id'");
	mysql_query("ALTER TABLE `clients` ADD `r".($currentid["val"]+1)."` VARCHAR( 500 ) NULL;");
}
function before_delete($id)
{
	$current_sql = db_query("SELECT * FROM clients_fields WHERE id = '$id'");
	$current = db_fetch_assoc($current_sql);
	db_query("ALTER TABLE `clients` DROP `$current[code]`");
