...
if(isset($_POST["action"])){
	switch($_POST["action"]){
		case "showGoodsPreview":
			...
			$filters=array();
			$filters_sql="";
			$filters_post=isset($_POST["filters"]) ? $_POST["filters"] : array();
			foreach($filters_post as $item){
				$filter=explode("=",$item);
				if((int)($filter[1])>0) $filters_sql.=" AND ".prepare($filter[0])."='".prepare($filter[1])."'";
				$filters[$filter[0]]=$filter[1];
			}
			$sql_text="SELECT goods.* FROM goods WHERE goodsid='".$maingoodsid."'".$filters_sql;
			$tovar=execute_row_assoc($sql_text);
			?>
			<div class="imagehandler">
				...
				<div class="visual-preview-box">
					...
					foreach($g_colors as $key=>$color){
						...
						if($tovar["r1193"]==$color["id"]) $current="curent"; else $current="";
						//$goodsOfColor=getRowsFromDB("SELECT images.* FROM images WHERE source=3 AND parentid IN (SELECT id FROM goods WHERE (goodsid=$maingoodsid AND r1193=$color[id]) OR id='$maingoodsid') LIMIT 0,5");
						$goodsOfColor=getRowsFromDB("SELECT images.* FROM images WHERE source=3 AND parentid IN (SELECT id FROM goods WHERE goodsid=$maingoodsid AND r1193=$color[id]) LIMIT 0,5");
						if(count($goodsOfColor)==0){
							$goodsOfColor=getRowsFromDB("SELECT images.* FROM images WHERE source=3 AND parentid='$maingoodsid' LIMIT 0,5");
						}
						?>
						<div class="visual-preview-box-item <?=$current?>" data-color-index="<?=$color["id"]?>">
							<div class="visual-preview-box-thumbs">
								...
								<div class="thumbs-vfix">
									<?$goodind=1;
									foreach($goodsOfColor as $goodimage){?>
					...
					<div class="stuff-dscr-menu jNice">
						<div class="stuff-dscr-color-box">
							<div class="curent-color">
								<?$ind=1;
								foreach($g_colors as $key=>$color){
									if($ind==1) $current="curent"; else $current="";?>
							...
							<div class="stuff-color-menu">
								<?foreach($g_colors as $key=>$color){?>
									<a style="background:<?=$color["code"]?>" data-color-index="<?=$color["id"]?>" href="javascript:setNewPreviewId('<?=$maingoodsid?>','<?=$tovar["id"]?>','r1193','<?=$color["id"]?>');"></a>
								<?}?>
							</div>
							<div class="clear"></div>
						</div>
	...
	<div style="position:absolute; top: -15px; right:-15px"><a onClick="$('#dropper').toggle();"><img style="border-radius: 5px" src="/templates/images/close_size.png" /></a></div>
					...
					<div class="stuff-buy-box" id="preview-buybtn-<?=$good["id"]?>">
						<input type="hidden" id="preview-r1193-<?=$maingoodsid?>" value="<?=$tovar["r1193"]?>" />
						<input type="hidden" id="preview-r1194-<?=$maingoodsid?>" value="<?=$tovar["r1194"]?>" />
		...
		case "getNewPreviewId":
			$goodsid=$_POST["maingoodsid"];
			$filters=array();
			$filters_sql="";
			$filters_post=isset($_POST["filters"]) ? $_POST["filters"] : array();
			foreach($filters_post as $item){
			...
			$sql_text="SELECT * FROM goods WHERE goodsid='".$goodsid."'".$filters_sql;
			echo $sql_text;
			$good=execute_row_assoc($sql_text);
			$maingoodsid=($good["goodsid"]>0) ? $good["goodsid"] : $good["id"];?>
			<input type="hidden" id="preview-r1193-<?=$maingoodsid?>" value="<?=$good["r1193"]?>" />
			<input type="hidden" id="preview-r1194-<?=$maingoodsid?>" value="<?=$good["r1194"]?>" />
