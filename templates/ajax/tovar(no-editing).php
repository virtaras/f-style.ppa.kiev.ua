		...
		case "showTovarView":
			...
			<?php
				...
				foreach ($colors as $k => $v) {
					$ind++;
					?>
					<a title="<?=$v["NAME"]?>" <?=($ind == 1 ? "class='curent'" : "")?> data-color-index="<?=$k?>" style="background:<?=$v['CODE']?>" id="c<?=$k?>" href="javascript:void(0);"></a>
					<?php } ?>
			...
			<?php
					...
					<div class="stuff-buy-box">
						<a class="button big" href="javascript:void(0);"
						   onclick="send_to_cart('<?=$tovar["id"]?>');">Купить</a>
			...
			<?php
				...
				foreach ($arr as $current) {
					...
					<script>
						var l = 0;
						$('#tovar-holder img').bind('load', function(){
						  l++;
						  if (l == $('#tovar-holder img').length) {
							 handleLoad();
						  }
						});			
					</script>
